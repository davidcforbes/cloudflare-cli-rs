...
All notable changes to CFAD (Cloudflare Admin CLI) will be documented
in this file.
...
- **D1 Database Support** - Full management of Cloudflare D1 SQLite databases
...
- **R2 Storage Support** - Full management of Cloudflare R2 object storage
  ...
  - `cfad r2 domain` - Custom domain management
  ...
  - `cfad r2 sippy` - Incremental migration from S3/GCS
  - `cfad r2 notifications` - Event notifications
  - `cfad r2 migrate` - Super Slurper bulk migration
...
- New table formatters for D1 and R2 resources

### Changed in 0.3.0

- **Account ID Configuration** - `--account-id` flag is now optional
...
- **Quality Check Claude Skill** - `/quality` command for checks
...
- Test coverage for DNS parsers, output formatters, configuration
...
- **Dependencies: 30% Reduction** (20 → 14 production dependencies)
  - Removed unused: chrono, futures, anyhow, colored, clap_complete
  - Removed unused dev deps: assert_cmd, predicates, tempfile
  - Updated to latest versions: reqwest 0.13, toml 0.9, dirs 6.0
  - Eliminated security warnings (RUSTSEC-2020-0159, RUSTSEC-2025-0055)

- **GitHub Actions: Modernized Workflows**
  - Release workflow: Replaced deprecated actions with modern versions
  ...
  - Removed musl build target (4 working platforms)
...
- **Zone Parameter** - All DNS commands now require zone parameter
...
The Cloudflare API requires both zone_id and record_id for DNS operations.
Requiring the zone parameter:
